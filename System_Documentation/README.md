...
- **`Transcript_Capture.md`** — `capture_transcript.py`, `clean_transcript.py`, `transcript_mcp_server.py`. The verbatim session archive: why capture needs a headed browser and cannot run in the Docker stack, why the page's HTML is stored rather than its visible text, the three DOM traps that produce plausible-looking wrong output, the heading levels retrieval depends on, and what breaks when claude.ai's page structure changes.
