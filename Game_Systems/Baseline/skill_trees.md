...
**Skip it when:**
- The PC begins with deep prior experience that predates the campaign
- The campaign is primarily social and political in structure, where consequences and relationships carry the dramatic weight
...
- A PC undergoing a transformation programme doesn't unlock "Transformation." They unlock a name drawn from their specific change — often the donor's name, or the form's.
...
These are starting points, not a taxonomy — name the branches from inside the character's experience. *The setting profile, `World_Building/[Setting]/Setting_Profile.md` > PROGRESSION, records what the setting's transformations do to Essence — that it initializes, when, and what carries over — and which campaigns a programme covers. It records no branch names, for the reason above: two characters through the same change grow different trees, and neither tree can be written down before the character has lived it. The campaign profile records only the per-campaign judgements.*
